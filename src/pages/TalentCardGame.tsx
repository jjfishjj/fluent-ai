...
const TUTORIAL_COPY: Record<Exclude<TutorialStep, 'intro' | null>, { progress: string; title: string; text: string }> = {
...
const TUTORIAL_TARGETS: Partial<Record<Exclude<TutorialStep, null>, string>> = {
  start: '[data-tutorial-target="start"]',
  map: '[data-tutorial-target="map"]',
  card: '[data-tutorial-target="card"]',
  input: '[data-tutorial-target="input"]',
  submit: '[data-tutorial-target="submit"]',
  feedback: '[data-tutorial-target="feedback"]',
};

interface TutorialTargetRect { top: number; left: number; right: number; bottom: number; width: number; height: number; }

function useTutorialTarget(step: TutorialStep) {
  const [targetRect, setTargetRect] = useState<TutorialTargetRect | null>(null);

  useEffect(() => {
    const selector = step ? TUTORIAL_TARGETS[step] : undefined;
    ...
    const rect = target.getBoundingClientRect();
    if (rect.top < 80 || rect.bottom > window.innerHeight - 160) target.scrollIntoView({ behavior: 'smooth', block: 'center' });
    updatePosition();
    const observer = new ResizeObserver(updatePosition);
    observer.observe(target);
    window.addEventListener('resize', updatePosition);
    window.addEventListener('scroll', updatePosition, true);
    const settleTimer = window.setTimeout(updatePosition, 450);
    return () => {
