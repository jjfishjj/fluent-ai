/**
 * MNEMO・VERSE 專用 3D 場景。
 *
 * 這是 practice/WebGLNumberScene 的獨立分支，只給 /mnemoverse 新遊戲頁使用。
 * 刻意不共用，讓這頁可以照 docs/mnemoverse-3d-game-design.md 的 P0~P5 持續演進，
 * 而不會動到既有的「03 3D 空間戰」分頁。
 *
 * 相對於原版新增：EffectComposer 後製堆疊、Combo 3/4/5 級空間效果。
 */
...
// typeface JSON 的推導型別和 drei 的 FontData 對不起來，實際結構相容
const helvetiker = helvetikerFont as unknown as Parameters<typeof Text3D>[0]['font'];
...
/** 連擊分級：3 級染整個艙、4 級開粒子環、5 級進入 OVERDRIVE。 */
const COMBO_ENV_SHIFT = 3;
const COMBO_RING = 4;
const COMBO_OVERDRIVE = 5;
...
/** Combo 4 級：加法混合粒子環繞玩家旋轉，連擊斷掉才收回。 */
function ComboRing({ color, active, overdrive }: { color: string; active: boolean; overdrive: boolean }) {
  const group = useRef<THREE.Group>(null);
  const count = 48;
  const positions = useMemo(() => {
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      array[index * 3] = Math.cos(angle) * 3.1;
      array[index * 3 + 1] = Math.sin(angle * 3) * .22;
      array[index * 3 + 2] = Math.sin(angle) * 3.1;
    }
    return array;
  }, []);
  ...
  useFrame((state, delta) => {
    ...
    group.current.scale.setScalar(THREE.MathUtils.damp(group.current.scale.x, target, 4.5, delta));
    group.current.rotation.y += delta * (overdrive ? 1.1 : .55);
    group.current.rotation.z = Math.sin(state.clock.elapsedTime * .4) * .12;
  });

  return (
    <group ref={group} scale={.001}>
      <points>
        <bufferGeometry><bufferAttribute attach="attributes-position" args={[positions, 3]} /></bufferGeometry>
        <pointsMaterial color={color} size={overdrive ? .13 : .085} transparent opacity={.9} depthWrite={false} blending={THREE.AdditiveBlending} />
      </points>
    </group>
  );
}

/** Combo 3 級：半球光色溫向該組 palette 位移，整個編碼艙被染色。 */
function EnvironmentShift({ palette, combo }: { palette: string; combo: number }) {
  const hemisphere = useRef<THREE.HemisphereLight>(null);
  const ambient = useRef<THREE.AmbientLight>(null);
  const neutral = useMemo(() => new THREE.Color('#c4f1ff'), []);
  const tint = useMemo(() => new THREE.Color(palette), [palette]);
  const blend = useRef(0);

  useFrame((_, delta) => {
    blend.current = THREE.MathUtils.damp(blend.current, combo >= COMBO_ENV_SHIFT ? 1 : 0, 2.4, delta);
    if (hemisphere.current) {
      hemisphere.current.color.copy(neutral).lerp(tint, blend.current * .85);
      hemisphere.current.intensity = 1.25 + blend.current * .65;
    }
    if (ambient.current) ambient.current.intensity = .35 + blend.current * .22;
  });

  return <>
    <ambientLight ref={ambient} intensity={.35} />
    <hemisphereLight ref={hemisphere} args={['#c4f1ff', '#110822', 1.25]} />
  </>;
}

/**
 * 後製堆疊。所有參數都由 useFrame 驅動，對應企劃的鏡頭六拍：
 * 拍 3 鎖定／拍 5 衝擊 → 色差撐開後收回、bloom 爆亮
 * 拍 4 輸入 → 背景散景加深，把注意力壓在物件上
 */
function PostFX({ submitted, combo, recallMode }: { submitted: boolean; combo: number; recallMode: boolean }) {
  const bloom = useRef<{ intensity: number }>(null);
  const aberration = useRef<{ offset: THREE.Vector2 }>(null);
  const depthOfField = useRef<{ bokehScale: number }>(null);
  const impact = useRef(0);

  useEffect(() => { if (submitted && !recallMode) impact.current = 1; }, [submitted, recallMode]);

  useFrame((_, delta) => {
    impact.current = THREE.MathUtils.damp(impact.current, 0, 4.2, delta);
    const overdrive = combo >= COMBO_OVERDRIVE ? 1.6 : 1;

    if (bloom.current) {
      // 回想艙壓低底值，維持「這裡沒有提示」的冷調
      const base = recallMode ? .55 : 1.05;
      bloom.current.intensity = THREE.MathUtils.damp(bloom.current.intensity, base * overdrive + impact.current * .8, 5, delta);
    }
    if (aberration.current) {
      const offset = impact.current * .0026;
      aberration.current.offset.set(offset, offset * .6);
    }
    if (depthOfField.current) {
      depthOfField.current.bokehScale = THREE.MathUtils.damp(depthOfField.current.bokehScale, submitted ? 1.4 : 3.2, 3, delta);
    }
  });

  return (
    <EffectComposer multisampling={4}>
      <DepthOfField ref={depthOfField as never} target={[0, 0, 0]} focalLength={.022} bokehScale={3.2} />
      <Bloom ref={bloom as never} intensity={1.05} luminanceThreshold={.22} luminanceSmoothing={.4} mipmapBlur radius={.72} />
      <ChromaticAberration ref={aberration as never} blendFunction={BlendFunction.NORMAL} offset={new THREE.Vector2(0, 0)} radialModulation modulationOffset={.35} />
      <Vignette darkness={.62} offset={.28} eskil={false} blendFunction={BlendFunction.NORMAL} />
    </EffectComposer>
  );
}
...
function Scene({ round, submitted, combo, quality, recallMode }: Props) {
  const overdrive = combo >= COMBO_OVERDRIVE;
  return <>
    <color attach="background" args={['#050a18']} />
    <fog attach="fog" args={['#050a18', 7, 19]} />
    <EnvironmentShift palette={round.palette} combo={combo} />
    ...
    <Stars radius={40} depth={18} count={quality === 'low' ? 450 : 1400} factor={2.2} saturation={.25} fade speed={overdrive ? 1.6 : .35} />
    ...
    <ComboRing color={round.palette} active={!recallMode && combo >= COMBO_RING} overdrive={overdrive} />
    ...
    {/* 省電模式完全不掛 composer，不付這個成本 */}
    {quality === 'high' && <PostFX submitted={submitted} combo={combo} recallMode={Boolean(recallMode)} />}
  </>;
}

export function MnemoVerseScene(props: Props) {
  return (
