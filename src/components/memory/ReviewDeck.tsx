...
export function ReviewDeck({ userId, eegEngagement }: { userId: string; eegEngagement?: number }) {
  ...
  const grade = async (item: MemoryItem, rating: Rating) => {
    ...
    await supabase.from('memory_items').update(toDB(next)).eq('id', item.id as string)
    await supabase.from('memory_reviews').insert({ memory_item_id: item.id, user_id: userId, quality: rating, review_modality: item.encoding_context, eeg_engagement: eegEngagement ?? null })
    const eng = eegEngagement != null ? `（腦波 ${Math.round(eegEngagement * 100)}%）` : ''
    toast.success(`📅 下次複習 ${intervalDays} 天後 · S=${next.stability.toFixed(1)}天 ${eng}`)
  ...
  if (!items.length) return (
    <div className="py-16 text-center">
      ...
      <p className="text-muted-foreground">今日 FSRS 複習已完成！</p>
    </div>
  )
      ...
      {items.map(item => {
        ...
        return (
          <div key={item.id as string} className="bg-card border border-border rounded-2xl p-4 relative">
            <span className="absolute top-3 right-3 text-xs px-2 py-0.5 rounded-full" style={{ background: mod.c + '22', color: mod.c }}>{mod.n}</span>
            ...
            <p className="text-xs text-muted-foreground mt-2">R={ret}% · S={card.stability.toFixed(1)}天 · D={card.difficulty.toFixed(1)}</p>
            <div className="h-1.5 bg-muted rounded-full mt-2 overflow-hidden">
              <div className="h-full rounded-full" style={{ width: ret + '%', background: ret < 50 ? '#ef4444' : mod.c }} />
            </div>
            <div className="grid grid-cols-4 gap-2 mt-3">
              {RATINGS.map(({ r, label, color }) => (
                <button key={r} onClick={() => grade(item, r)} className="py-2 border border-border rounded-xl text-xs font-semibold bg-background hover:bg-muted transition-colors" style={{ color }}>{label}</button>
              ))}
            </div>
          </div>
        )
