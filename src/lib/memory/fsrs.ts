...
export function review(card: FSRSCard, rating: Rating, now: Date = new Date(), opts: ReviewOpts = {}): { card: FSRSCard; intervalDays: number } {
  ...
  if (card.state === 'new') {
    ...
    if (opts.eegEncoding != null) {
      next.stability *= (1 + opts.eegEncoding * 0.6)
    }
...
export function fromDB(row: Record<string, unknown>): FSRSCard {
  return {
    ...
    state:      (row.fsrs_state      as FSRSCard['state']) ?? 'new',
    lapses:     (row.lapses          as number) ?? 0,
    reps:       (row.repetitions     as number) ?? 0,
    due:        new Date(row.next_review_at as string),
    lastReview: row.last_reviewed_at ? new Date(row.last_reviewed_at as string) : null,
  }
}
