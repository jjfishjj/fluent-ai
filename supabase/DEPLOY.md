...
```bash
npx supabase functions deploy chat
npx supabase functions deploy chat-image      # AI 生成畫面（記憶宮殿／對話中生圖）也吃同一組 AI_API_KEY
npx supabase functions deploy analyze-content  # 分享連結／YouTube 影片解析
```
確認目前設了什麼：`npx supabase secrets list`。

> `chat-image`（記憶宮殿「AI 生成畫面」、對話中生圖）用同一把 `AI_API_KEY`，預設圖片模型是
> `gemini-2.5-flash-image`。Claude 沒有生圖 API，所以這個功能只能走 Gemini 或其他支援生圖的
> OpenAI 相容供應商。若該模型名稱之後被 Google 改名/下架導致生圖失敗（回傳「model not
> available」），不用改 code，直接：
> ```bash
> npx supabase secrets set AI_IMAGE_MODEL=<新的模型名>
> npx supabase functions deploy chat-image
> ```
...
不用真的 fine-tune 模型。到 App 的 **`/admin` → 訓練資料** 分頁，直接新增/編輯/刪除
問答範例（可選擇限定語言、限定記憶天才型態，或不限），`chat` function 每次對話都會抓
啟用中、符合語言/型態的資料當作參考範例塞進去，藉此讓機器人的回答風格照你輸入的方向調整。
這一步**不需要**額外設定 secrets——`SUPABASE_URL` / `SUPABASE_SERVICE_ROLE_KEY` 是 Supabase
自動注入給每個 Edge Function 的環境變數。改了 `qa_training_examples` 表的資料立即生效，
不用重新部署 function；只有改 `chat/index.ts` 的 code 才需要 `npx supabase functions deploy chat`。
